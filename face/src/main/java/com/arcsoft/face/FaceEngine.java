...
public class FaceEngine {
    ...
    private native int activeFaceEngineOnline(Context var1, String var2, String var3);

    private native int getActiveFile(Context var1, ActiveFileInfo var2);
    ...
    private native void processIr(long var1, byte[] var3, int var4, int var5, int var6, FaceInfo[] var7, int var8, ErrorInfo var9);
    ...
    private native int setLivenessParam(long var1, LivenessParam var3);

    private native int getLiveness(long var1, LivenessInfo[] var3, ErrorInfo var4);

    private native int getIrLiveness(long var1, LivenessInfo[] var3, ErrorInfo var4);
    ...
    public int activeOnline(Context context, String appId, String sdkKey) {
        return context != null && appId != null && sdkKey != null ? this.activeFaceEngineOnline(context, appId, sdkKey) : 2;
    }

    public int getActiveFileInfo(Context context, ActiveFileInfo activeFileInfo) {
        return activeFileInfo == null ? 2 : this.getActiveFile(context, activeFileInfo);
    }
    ...
    public int detectFaces(byte[] data, int width, int height, int format, List<FaceInfo> faceInfoList) {
        if (format != 2050 && format != 513 && format != 1793 && format != 3074) {
            ...
            if (faceInfoList != null && data != null) {
                if (!this.isImageDataValid(data, width, height, format)) {
                    return 86021;
    ...
    public int processIr(byte[] data, int width, int height, int format, List<FaceInfo> faceInfoList, int combineMask) {
        if (format != 2050 && format != 1793 && format != 3074) {
            ...
            if (faceInfoList != null && data != null) {
                if (!this.isImageDataValid(data, width, height, format)) {
                    ...
                    this.processIr(this.handle, data, width, height, format, (FaceInfo[])faceInfoList.toArray(new FaceInfo[0]), combineMask, this.error);
                    return this.error.code;
                }
            } else {
                return 2;
    ...
    public int setLivenessParam(LivenessParam livenessParam) {
        if (livenessParam != null && livenessParam.getIrThreshold() <= 1.0F && livenessParam.getIrThreshold() >= 0.0F && livenessParam.getRgbThreshold() <= 1.0F && livenessParam.getRgbThreshold() >= 0.0F) {
            return this.handle == 0L ? 5 : this.setLivenessParam(this.handle, livenessParam);
        } else {
            return 2;
        }
    }
    ...
    public int getIrLiveness(List<LivenessInfo> irLivenessInfoList) {
        if (irLivenessInfoList == null) {
            ...
            int count = this.getIrLiveness(this.handle, this.mIrLivenessInfoArray, this.error);
    ...
    private boolean isImageDataValid(byte[] data, int width, int height, int format) {
        return format == 2050 && data.length == width * height * 3 / 2 || format == 513 && data.length == width * height * 3 || format == 1793 && data.length == width * height || format == 3074 && data.length == width * height * 2;
    }
